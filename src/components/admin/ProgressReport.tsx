...
// import { AlertCircle, Download, BarChart2, AlertTriangle } from 'lucide-react';
...
//         const { data } = await apiClient.getProgressReport(selectedSheetId);
...
//       // Create CSV rows
//       const rows = progressData.map(entry => [
//         entry.userName,
//         entry.userEmail,
//         `${entry.completionPercentage}%`,
//         entry.completedProblems.toString(),
//         entry.totalProblems.toString(),
//         `${entry.byDifficulty.Easy.completed}/${entry.byDifficulty.Easy.total}`,
//         `${entry.byDifficulty.Medium.completed}/${entry.byDifficulty.Medium.total}`,
//         `${entry.byDifficulty.Hard.completed}/${entry.byDifficulty.Hard.total}`
//       ]);
...
//       // Get the current sheet name for the filename
//       const sheetName = sheets.find(s => s._id === selectedSheetId)?.title || 'progress-report';
//       const safeSheetName = sheetName.replace(/[^a-z0-9]/gi, '-').toLowerCase();

//       link.setAttribute('download', `${safeSheetName}-progress-${new Date().toISOString().slice(0, 10)}.csv`);
...
//             disabled={!selectedSheetId || progressData.length === 0 || exportLoading}
//             onClick={exportProgressAsCSV}
//           >
//             {exportLoading ? (
//               <>
...
//             <h3 className="text-lg font-medium mb-1">No progress data available</h3>
//             <p className="text-sm text-center max-w-md">
//               No users have started working on this sheet yet. Progress data will appear here once users begin solving problems.
//             </p>
//           </div>
//         ) : (
